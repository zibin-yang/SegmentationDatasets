...
class ClassinSeg(data.Dataset):
    ...
    def __init__(self, root, train=True, transform=None):
        self.root = root
        _list_dir = os.path.join(root, 'segmentation')
...
def main():
    root = "../data/ClassinSeg"
    classinSeg = ClassinSeg(root, train=True)

    for i in range(20):
        image, label = classinSeg[i]
